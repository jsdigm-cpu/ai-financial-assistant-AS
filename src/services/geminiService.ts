...
★ 매출 패턴:
- 배달의민족·쿠팡이츠·요기요·배민·땡겨요·먹깨비 입금 → 배달매출 / 출금 → 배달수수료
- VAN정산·카드정산·POS정산·시흥정산 입금 → 카드매출
- "카드사명+숫자코드" 형태 입금(예: 삼성197963861, KB11610154, 신한13727349, 현142302158, NH17434147, 하나921404753, 우601987000, 롯데37865250, 739447034B 등) → 카드매출 (VAN사 카드사별 정산 패턴)
- 네이버페이·Npay·카카오페이·토스·엔에이치엔페이·페이코 입금 → 간편결제
- 제로페이·지역사랑상품권·온누리상품권 입금 → 지역화폐
- 스마트스토어·쿠팡판매·11번가 입금 → 입점몰매출

★ 환급·환불 입금 (반드시 수입 카테고리):
- "환급", "환불", "수수료환급", "우대환급", "KB환급", "삼성환급", "캐쉬백", "캐시백" 등 → 보험금·환급금
- "정부지원", "지원금", "보조금", "배달택배비지원" → 정부지원금
- "예금이자", "적금이자" → 이자수익

★ 보험사 패턴:
- "한화생명", "삼성생명", "교보생명" 등 + 출금 → 보험료 (지출)
- "한화생명", "삼성생명", "교보생명" 등 + 입금 → 보험금·환급금 (수입)

★ 대출/금융:
- "대출실행", "한도대출" 입금 → 차입금
- "원금상환", "대출이자" 출금 → 대출원금상환 / 이자비용
- "삼성카드", "하나카드", "신한카드대금", "카드대금" 출금 → 신용카드대금

★ 세금:
- "부가가치세", "부가세", "세외수입", "세외수납", "국세" 출금 → 세금·공과금
- "국민연금", "건강보험", "고용보험", "산재보험" 출금 → 세금·공과금

★ 재료·식자재 납품업체:
- "XX푸드", "XX식품", "XX유통" 형태 출금 → 원재료(식자재) (식재료 납품업체)
- "XX주류", "광성주류" 등 출금 → 부자재(주류)

★ 개인 지출:
- 병원·치과·의원·한의원·약국 출금 → 병원·약국비
- 주유·하이패스·고속도로 출금 → 유류비·교통비
- 가스(도시가스, 가스비, 가스요금) 출금 → 도시가스비
