...
const DashboardView: React.FC<Props> = ({ transactions, businessInfo, categories }) => {
  ...
  return (
    <div className="space-y-10 max-w-7xl mx-auto">
      {/* 헤더 + 기간 선택 */}
      <div className="relative flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 p-6 md:p-8 rounded-3xl shadow-sm border border-border-color overflow-hidden">
        <img 
        ...
        <div className="relative z-10 w-full flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
          <div>
            ...
            <p className="mt-3 text-lg text-text-muted font-medium">통장 거래 내역을 바탕으로 수입과 지출을 한눈에 정리한 요약 화면입니다.</p>
          </div>
          <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
            ...
            <div className="relative flex-1 sm:flex-none">
              <select
                ...
                className="w-full sm:w-auto pl-4 pr-10 py-3 bg-white/80 backdrop-blur-md text-text-primary border-2 border-border-color rounded-xl text-base font-bold focus:ring-4 focus:ring-brand-primary/20 focus:border-brand-primary appearance-none cursor-pointer transition-all shadow-sm"
              >
      ...
      {/* 월별 차트 */}
      <div className="bg-white p-8 rounded-3xl shadow-sm border border-border-color">
        <SectionTitle title="월별 재무 성과 분석" icon="📈" desc="월별 수입(파란색)과 지출(빨간색) 추이를 막대 그래프로 보여줍니다. 어느 달에 돈이 많이 들어오고 나갔는지 비교해 보세요." />
        <div className="mt-6">
